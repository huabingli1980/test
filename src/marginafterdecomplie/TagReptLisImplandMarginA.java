...
public class TagReptLisImplandMarginA implements TagReportListener {
				...
				try {
					if (null != marginW.RorW.getSelectedItem().toString()) {
						...
						case "WriteTid":
							...
							// void programTid(String currentTid, String
							// newTid,String CurrentEpc, ImpinjReader reader)
							break;
						case "LockOnly":
						...
						case "UnLock":
							...
							// LockEpcMemory.UnLockMemory("8AF843B8",
							// currentTid, reader);
	...
	private class MarginEpc implements TagOpCompleteListener {
		...
		void programEpc(String currentEpc, short currentPC, String newEpc, ImpinjReader reader) throws Exception {
		...
		void programTid(String currentTid, String newTid, String CurrentEpc, ImpinjReader reader) throws Exception {

			if ((currentTid.length() % 4 != 0) || (newTid.length() % 4 != 0)) {
				throw new Exception("EPCs must be a multiple of 16- bits: " + currentTid + "  " + newTid);
			}
		...
		public synchronized void onTagOpComplete(ImpinjReader reader, TagOpReport results) {
			...
			results.getResults().forEach((TagOpResult t) -> {
				if ((t instanceof TagMarginReadOpResult)) {
					...
					System.out.println(
							"Margin Read Complete (" + mrResult.getTag().getEpc() + ")--- " + mrResult.getResult());
					...
					if ("InsufficientPower".equals(result)) {
						...
						new Thread(() -> {
							...
							// String[]{marginW.ListT.getRowCount() + 1 + "",
							// tid, epc, "", result + "---" + times + "times",
							// chipType, nowtime});
							...
							marginW.ListT.scrollRectToVisible(rect);
							marginW.ListT.setRowSelectionInterval(marginW.ListT.getRowCount() - 1,
									marginW.ListT.getRowCount() - 1);
					...
					if (t instanceof TagWriteOpResult) {
						...
						System.out.println(
								" result: " + tr.getResult().toString() + " words_written: " + tr.getNumWordsWritten());
						outstanding--;
					}
